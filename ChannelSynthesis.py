...
dataset_list = ['Pathway2_3']#,'Pathway2_1','Pathway2_2','Pathway2_3','Bus_2',
                #'Staircase_1','Staircase_2','Staircase_3',
                 #  'Staircase_4','Doorway_1','Doorway_2','Doorway_3','Room_1','Room_2']
folder = 'C:/Users/superorange/Google 雲端硬碟/NCHC_backup/2018/Detect_Results'
...
if isMDtrack:
    ...
    folder = 'C:/Users/superorange/Google 雲端硬碟/NCHC_backup/2018/MDtrack_Results'
...
if __name__ == '__main__':
    ...
    output_type =2  #1: output 1 folder/1 txt each image, 2: output 1 file

    for method in method_list:
        for dataset in dataset_list:

            if output_type==1:
                output_folder = os.path.join(folder,method+'_' + dataset +folder_concat_str+ '_merge')
            ...
            for idx in in_dict:
                if output_type == 1:
                    output_file_name = dataset + '_' + str(idx).zfill(5) + ".txt"
                    if isFace:
                        ...
                        for A_idx, bbA in enumerate(in_dict[idx]["channel" + str(ch_tuple[0])]):
                            for B_idx, bbB in enumerate(in_dict[idx]["channel" + str(ch_tuple[1])]):
                                if bbA["type"] == bbB["type"]:
                                    iou = BoundingBoxIOU.getIOU(bbA["bbox"], bbB["bbox"])
                                    if iou > IOU_th:
